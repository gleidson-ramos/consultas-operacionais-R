  ...
  output$tabela_agendados <- renderTable({
    dados <- dbGetQuery(con, "
      SELECT
        ...
        IFNULL(a.observacao, '—')                 AS Observação
      ...
      WHERE DATE(a.data_agendada) = CURDATE()
  ...
  output$tabela_andamento <- renderTable({
    dados <- dbGetQuery(con, "
      SELECT
        ...
        CONCAT(
          TIMESTAMPDIFF(HOUR, a.data_inicio, NOW()), 'h ',
          MOD(TIMESTAMPDIFF(MINUTE, a.data_inicio, NOW()), 60), 'min'
        )                                                       AS `Tempo Decorrido`
      FROM atendimento_coleta a
      JOIN ocorrencia o       ON o.id_ocorrencia = a.id_ocorrencia
      JOIN ponto_monitorado p ON p.id_ponto       = o.id_ponto
  ...
  output$tabela_atrasados <- renderTable({
    dados <- dbGetQuery(con, "
      SELECT
        ...
        CONCAT(
          TIMESTAMPDIFF(HOUR, a.data_agendada, NOW()), 'h ',
          MOD(TIMESTAMPDIFF(MINUTE, a.data_agendada, NOW()), 60), 'min'
        )                                                         AS `Atraso`
      FROM atendimento_coleta a
      JOIN ocorrencia o       ON o.id_ocorrencia = a.id_ocorrencia
      JOIN ponto_monitorado p ON p.id_ponto       = o.id_ponto
  ...
  output$tabela_sem_responsavel <- renderTable({
    dados <- dbGetQuery(con, "
      SELECT
        ...
        o.prioridade                                             AS Prioridade,
        CONCAT(
          TIMESTAMPDIFF(HOUR, o.data_abertura, NOW()), 'h em aberto'
        )                                                        AS `Tempo Aberta`
      FROM ocorrencia o
      JOIN ponto_monitorado p ON p.id_ponto      = o.id_ponto
      JOIN logradouro l       ON l.id_logradouro  = p.id_logradouro
